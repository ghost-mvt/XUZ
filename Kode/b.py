...
class EngineController:
    ...
    def search_by_name_and_city(self):
        ...
        first_name = input(f" {Theme.RED_D}╠═ {Theme.TEXT}FIRST NAME : {Theme.WHITE}").strip().upper()
        surname    = input(f" {Theme.RED_D}╠═ {Theme.TEXT}SURNAME    : {Theme.WHITE}").strip().upper()
        target_il  = input(f" {Theme.RED_D}╚═ {Theme.TEXT}NUFUSIL    : {Theme.WHITE}").strip().upper()
        if not first_name or not surname: return
        
        UIComponents.progress_bar("streaming full database nominal records", duration=0.5)
        try:
            # تم حذف LIMIT 10 لجلب جميع الأسماء المتطابقة بالكامل بدون قيود
            query = "SELECT * FROM `101m` WHERE `ADI` = %s AND `SOYADI` = %s" + (" AND `NUFUSIL` = %s" if target_il else "")
            params = (first_name, surname, target_il) if target_il else (first_name, surname)
            all_records = self.db.execute_query(query, params, fetch_all=True)
            
            if not all_records:
                print(f" {Theme.STEALTH}[{Theme.WARN}-{Theme.STEALTH}] {Theme.TEXT}Zero matches located.{Theme.RESET}\n")
                return

            # تشغيل ميزة الفلترة الفورية الديناميكية داخل الذاكرة
            ...
            if filter_choice in ['y', 'yes']:
                ...
                # تطبيق الفلترة بشكل متقاطع فوري
                if f_baba:
